  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        {/* Filters Sidebar */}
        <div className="w-full md:w-64 shrink-0 md:sticky md:top-24 h-fit space-y-6">
          <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <h3 className="font-heading font-semibold text-lg flex items-center gap-2 mb-4 text-slate-900 dark:text-white">
              <Filter className="h-5 w-5 text-primary" /> {t('home.filters')}
            </h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('home.category')}</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('home.condition')}</label>
                <select 
                  className="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm dark:text-white focus:ring-primary"
                  value={searchParams.condition}
                  onChange={(e) => setSearchParams(prev => ({...prev, condition: e.target.value, page: 0}))}
                >
                  <option value="">{t('home.allConditions')}</option>
                  {conditions.map(c => <option key={c} value={c}>{c.charAt(0) + c.slice(1).toLowerCase()}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('home.priceRange')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('home.sortBy')}</label>
                <select 
                  className="w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm dark:text-white focus:ring-primary"
                ...
                >
                  ...
                  <option value="CONDITION">{t('home.conditionSort')}</option>
                </select>
              </div>
        ...
        <div className="flex-1">
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              ...
              <h3 className="text-lg font-medium text-slate-900 dark:text-white">{t('home.noBooksFound')}</h3>
              <p className="text-slate-500 dark:text-slate-400 mt-2">{t('home.noBooksFoundHint')}</p>
            </div>
          ) : (
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-end justify-between mt-auto mb-4">
                        ...
                        <span className="text-xs font-medium bg-primary/10 text-primary px-2 py-1 rounded-md">
                          {Math.round(((book.mrp - book.price) / book.mrp) * 100)}% {t('home.off')}
                        </span>
                      </div>
                      ...
                      {isInCart(book.id) ? (
                        ...
                        >
                          {t('home.goToCart')} <ArrowRight className="h-4 w-4" />
                        </button>
                      ) : (
                        <button
                        ...
                        >
                          {(addToCartMutation.isPending && addToCartMutation.variables === book.id) ? (
                            ...
                            <>
                              <ShoppingCart className="h-4 w-4" /> {t('home.addToCart')}
                            </>
                          )}
                        </button>
                      )}
              ...
              {data?.totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-700 pt-6 mt-8">
                  ...
                  <span className="text-sm text-slate-600 dark:text-slate-400">
                    {t('home.page')} <span className="font-medium text-slate-900 dark:text-white">{data.number + 1}</span> {t('home.of')} <span className="font-medium text-slate-900 dark:text-white">{data.totalPages}</span>
                  </span>
                  <button
                    onClick={() => handlePageChange(data.number + 1)}
